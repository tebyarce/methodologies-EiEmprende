...
function CompetenciasBig({ comp_indexes = [] }) {
  ...
  const handleChange = (event, newValue) => {
    ...
    useEffect(() => {
      if (!comp_indexes.includes(value+1)) {
        setValue(comp_indexes[0] - 1);
      }
    });
  ...
  return (<Box
    ...
    <Tabs
      ...
      value={value}
      sx={{ borderRight: 1.2, borderColor: "black", width: "45%", justifyContent: "center",}}>
      {competencias.map((_, comp_index) => (
        <Tab label={competencias[comp_index].name}
          {...a11yProps(comp_index)} 
          hidden={!comp_indexes.includes(comp_index + 1)}>
        </Tab>
      ))}
    </Tabs>
    {comp_indexes.map(comp_index => (
      <TabPanel value={value} index={comp_index - 1} width="200px">
        <p>{competencias[comp_index - 1].description}</p>
        <b>Resultados de aprendizaje</b>
        <ol>
          {competencias[comp_index - 1].resultados.map(resul => (
            <li>{resul}</li>
          ))}
        </ol>
      </TabPanel>
...
export default CompetenciasBig;
